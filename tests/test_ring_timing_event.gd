...
static func test_crushing_blow_windows(t: TestRunner) -> void:
	var ring := _make_ring(1800.0, 1.5, 1.5, [1, 1.15, 1.3, 3.6])
...
static func test_fireball_windows(t: TestRunner) -> void:
	var ring := _make_ring(1400.0, 1.0, 2.5, [1, 1.25, 1.5, 2])
...
static func test_dagger_throw_windows(t: TestRunner) -> void:
	var ring := _make_ring(1100.0, 0.7, 4.0, [1, 1.5, 2, 3])
...
static func test_miss_deals_exactly_base_damage(t: TestRunner) -> void:
	# "A miss still does the card's base effect" -- DESIGN.md section 4.
	var ring := _make_ring(1400.0, 1.0, 2.5, [1, 1.25, 1.5, 2])
	t.assert_eq(ring.damage_for_tier(8, RingTimingEvent.Tier.MISS), 8, "miss deals exactly base damage, never less")
...
static func run_all(t: TestRunner) -> void:
	...
	t.current_test = "test_miss_deals_exactly_base_damage"
	test_miss_deals_exactly_base_damage(t)
